...
export function AssessmentList({ jobs, assessment }: CandidatesListProps) {
  ...
  const moveCandidateToNextStep = async (
    ...
    await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/update/candidate-assessment/${candidateAssessmentId}/${candidateId}/job/${jobs[0].id}`,
      {
        method: "POST",
        headers: {
      ...
      <table className="w-full">
        ...
        <tbody>
          {candidates
            .filter(
              (candidate) =>
                assessmentDataCandidates[candidate.id]?.status !== "completed"
            )
            .map((candidate) => (
              <tr key={candidate.id} className="border-b hover:bg-gray-50">
                ...
                <td className="py-4">
                  <div>
                    <div>
                      {assessmentDataCandidates[candidate.id]?.status || "New"}
                    </div>
                    <div className="text-sm text-gray-600">
                  ...
                  <div className="flex flex-col space-y-2">
                    <div className="flex flex-col justify-between">
                      <span className="text-sm font-medium py-1 rounded">
                        Score:{" "}
                        {assessmentDataCandidates[candidate.id]?.status ===
                        "completed"
                          ? assessmentDataCandidates[
                              candidate.id
                          ...
                          "pending" ? (
                            <div className="text-center py-2">
                              <p className="text-sm text-gray-600">
                                Yet to take assessment
                              </p>
                              {assessmentDataCandidates[candidate.id]
                                ?.started_at && (
                          ...
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                console.log(
                                  `Moving candidate ${candidate.id} ${candidate.candidate_assessment_id} to next step`
                                );
                                try {
